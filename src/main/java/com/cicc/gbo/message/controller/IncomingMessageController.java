...
@Controller
@RequestMapping(value = "/incomingMessage")
public class IncomingMessageController extends BaseController {

    public static Logger logger = LoggerFactory.getLogger(IncomingMessageController.class);
	...
	public String listIncomingMessage(HttpServletRequest request, Model model) throws Exception {
		readGridSetting(request, model);
		clearComparatorList();
		addWebAttributes(model);
		return "incomingMessage/incomingMessageList";
	}
	
	@RequestMapping(method = RequestMethod.GET, value = "load")
	public String loadIncomingMessage(HttpServletRequest request, @ModelAttribute("incomingMessage") IncomingMessage incomingMessage, Model model) throws Exception {
		List<IncomingMessage> incomingMessageList = incomingMessageService.listByExample(incomingMessage, initJqgridPagination(request, model));
		
		model.addAttribute("entityList", incomingMessageList);				
		model.addAttribute("actionPagination", getActionPagination(request, model));
		return "incomingMessage/xmlIncomingMessageList";
	}
	
	@RequestMapping(method = RequestMethod.POST, value = "create")
	public String createIncomingMessage(HttpServletRequest request, Model model) {
		model.addAttribute("incomingMessage", new IncomingMessage());
		addWebAttributes(model);
		return "incomingMessage/incomingMessageDetail";
	}
	
	@RequestMapping(method = RequestMethod.POST, value = "read/{id}")
	public String readIncomingMessage(HttpServletRequest request, @PathVariable("id") Long id, Model model) {
		model.addAttribute("incomingMessage", incomingMessageService.read(id));
		addWebAttributes(model);
		return "incomingMessage/incomingMessageDetail";
	}
	
	@RequestMapping(method = RequestMethod.POST, value = "update")
	public void updateIncomingMessage(HttpServletResponse response, @ModelAttribute("incomingMessage") IncomingMessage incomingMessage, Model model) throws Exception {
		try{		
			Long id = incomingMessage.getEntityId();
			if(id == null) {
				incomingMessageService.create(incomingMessage);
			} else {
				incomingMessageService.update(incomingMessage);
			}
		}catch(Exception e){
	...
	@ResponseBody
	public void deleteIncomingMessage(HttpServletResponse response, @PathVariable("id") String ids) throws Exception {
		String[]idArray = ids.split(",");
		for(String idStr : idArray){
			Long id = Long.valueOf(idStr);
			try{
				incomingMessageService.delete(id);
	...
	@ModelAttribute
	public void getIncomingMessage(@RequestParam(value = "id", defaultValue = "-1") Long id, Model model) {
		if (id != -1) {
			model.addAttribute("incomingMessage", incomingMessageService.read(id));
		}
	}
	...
	public String upload(HttpServletRequest request, Model model) throws Exception {
		...
		return "incomingMessage/incomingMessageUpload";
	}
	...
	public void uploadFile(MultipartHttpServletRequest request,
		...
		try {
			...
			ImportResultObj importResultObj = ob.covertToObject(IncomingMessage.class);
			...
			if(importResultObj.getWrongObjList().size() != 0){
				...
				for(WrongMsgObj wrongMsgObj : importResultObj.getWrongObjList()){
					IncomingMessage wrongIncomingMessage = (IncomingMessage) wrongMsgObj.getObject();
					String errorMsg = wrongMsgObj.getMessage();
					failureRecord = failureRecord + JsonUtil.toJson(wrongIncomingMessage)+ " Error Msg: " + errorMsg + "\n";
				}
			}
			@SuppressWarnings("unchecked")
			List<IncomingMessage> incomingMessageList = (List<IncomingMessage>) importResultObj.getRightObjList();
			for(IncomingMessage each : incomingMessageList){
				try{
					incomingMessageService.create(each);
					successRecordNum++;
				}catch(Exception e){
					successFlag  = false;
					failureRecordNum++;
