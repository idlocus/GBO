...
@Controller
@RequestMapping(value = "/figuration")
public class FigurationController extends BaseController {

    public static Logger logger = LoggerFactory.getLogger(FigurationController.class);
	...
	public String listFiguration(HttpServletRequest request, Model model) throws Exception {
		readGridSetting(request, model);
		clearComparatorList();
		addWebAttributes(model);
		return "figuration/figurationList";
	}
	
	@RequestMapping(method = RequestMethod.GET, value = "load")
	public String loadFiguration(HttpServletRequest request, @ModelAttribute("figuration") Figuration figuration, Model model) throws Exception {
		List<Figuration> figurationList = figurationService.listByExample(figuration, initJqgridPagination(request, model));
		
		model.addAttribute("entityList", figurationList);				
		model.addAttribute("actionPagination", getActionPagination(request, model));
		return "figuration/xmlFigurationList";
	}
	
	@RequestMapping(method = RequestMethod.POST, value = "create")
	public String createFiguration(HttpServletRequest request, Model model) {
		model.addAttribute("figuration", new Figuration());
		addWebAttributes(model);
		return "figuration/figurationDetail";
	}
	
	@RequestMapping(method = RequestMethod.POST, value = "read/{id}")
	public String readFiguration(HttpServletRequest request, @PathVariable("id") Long id, Model model) {
		model.addAttribute("figuration", figurationService.read(id));
		addWebAttributes(model);
		return "figuration/figurationDetail";
	}
	
	@RequestMapping(method = RequestMethod.POST, value = "update")
	public void updateFiguration(HttpServletResponse response, @ModelAttribute("figuration") Figuration figuration, Model model) throws Exception {
		try{		
			Long id = figuration.getEntityId();
			if(id == null) {
				figurationService.create(figuration);
			} else {
				figurationService.update(figuration);
			}
		}catch(Exception e){
	...
	@ResponseBody
	public void deleteFiguration(HttpServletResponse response, @PathVariable("id") String ids) throws Exception {
		String[]idArray = ids.split(",");
		for(String idStr : idArray){
			Long id = Long.valueOf(idStr);
			try{
				figurationService.delete(id);
	...
	@ModelAttribute
	public void getFiguration(@RequestParam(value = "id", defaultValue = "-1") Long id, Model model) {
		if (id != -1) {
			model.addAttribute("figuration", figurationService.read(id));
		}
	}
	...
	public String upload(HttpServletRequest request, Model model) throws Exception {
		...
		return "figuration/figurationUpload";
	}
	...
	public void uploadFile(MultipartHttpServletRequest request,
		...
		try {
			...
			ImportResultObj importResultObj = ob.covertToObject(Figuration.class);
			...
			if(importResultObj.getWrongObjList().size() != 0){
				...
				for(WrongMsgObj wrongMsgObj : importResultObj.getWrongObjList()){
					Figuration wrongFiguration = (Figuration) wrongMsgObj.getObject();
					String errorMsg = wrongMsgObj.getMessage();
					failureRecord = failureRecord + JsonUtil.toJson(wrongFiguration)+ " Error Msg: " + errorMsg + "\n";
				}
			}
			@SuppressWarnings("unchecked")
			List<Figuration> figurationList = (List<Figuration>) importResultObj.getRightObjList();
			for(Figuration each : figurationList){
				try{
					figurationService.create(each);
					successRecordNum++;
				}catch(Exception e){
					successFlag  = false;
					failureRecordNum++;
