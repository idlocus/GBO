...
<%
	...
	List<LocationView> roleTree = (List<LocationView>) results
			.get("roleTree");
	...
	Long currentMenuId = (Long) request
			.getAttribute(Constants.CURRENT_MENU_ID);
%>
...
<script type="text/javascript">
	function submitForm() {
		...
		GafAjax.post(contextPathEnriched + "/app/privilege/role", parameters,
				addSuccess, 'JSON', addError);
	}
	...
	function refreshMenuTreeForRole(roleNode) {
		...
		var contextPathEnriched = App.getAppContextPathEnriched();
		window.location.href = contextPathEnriched + "/app/privilege/role/"
				+ roleId + "?menuId=" + <%=currentMenuId%>;
...
<body>
				...
				<form action="#" method="post" id="mainForm">
					<div id="main" style="padding: 10px; width: 800px; border: 0px grey solid; height:580px; margin-top: 10px;">
						<div id="line" style="width: 800px; height:580px; margin-top: 10px;">
							<div id="role"
								style="height:580px; width: 375px; border: 2px grey solid; float: left; margin-left: 20px; overflow: auto">
								<H1>Roles</H1>
								<ul id="roleTree" class="filetree treeview-famfamfam">
									...
									<li class="closed"><span class="folder">&nbsp;<%=roleDataMap.getLocationName()%></span>
										...
										<%
											if (roleDataMap.getDepartments() != null
														&& roleDataMap.getDepartments().size() > 0) {
													List<DepartmentView> deptList = roleDataMap
															.getDepartments();
										%>
										<ul>
											<%
											...
											<li><span class="folder">&nbsp;<%=dept.getDepartmentName()%></span>
												...
												<%
													if (dept.getOperators() != null
																		&& dept.getOperators().size() > 0) {
												%>
												<ul>
													<%
													...
													%>
													<li><span class="file" name="roleLi">&nbsp;<input
															type="radio" checked name="role"
															value="<%=role.getRoleID()%>"
															onClick="refreshMenuTreeForRole(this)" /> <%=role.getRoleName()%></span>
													</li>
													<%
														} else {
													%>
													<li><span class="file">&nbsp;<input
															type="radio" name="role" value="<%=role.getRoleID()%>"
															onClick="refreshMenuTreeForRole(this)" /> <%=role.getRoleName()%></span>
													</li>
													<%
														}
 ...
 %>
							...
							<div id="menu"
								style="height:580px; width: 375px; border: 2px grey solid; float: left; margin-left: 10px; overflow: auto">
								<H1>Menus</H1>
								<%=grantingMenu%>
							</div>
						</div>
		...
		boolean isHasPriv = logonService.isUserHasPrivilegesWrapper("/privilege/role", "POST");
		if (isHasPriv)
		{
		%>
